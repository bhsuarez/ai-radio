...
<body>
  ...
  <aside class="sidebar">
    <h1>🎧 AI Radio</h1>
    <a class="btn" id="openLink" href="#" target="_blank" rel="noopener">🔗 Open Stream</a>
    <a class="btn" href="http://icecast.zorro.local:8000" target="_blank">📊 Icecast Admin</a>
    <audio id="livePlayer" controls preload="none"></audio>
    <button class="btn warn" id="skipBtn">⏭️ Skip Track</button>
    <button class="btn accent" id="genBtn">✨ Generate DJ Line</button>
    <button class="btn" id="debugDjBtn">🔍 Debug DJ Status</button>
  ...
  <div class="main">
    <header>
      <div class="title">📜 Live Timeline</div>
    </header>
                ...
                try {
                  ...
                  if (response.ok) {
                    ...
                    if (result.found_file && result.has_artwork) {
                      ...
                      console.log(`✅ Found artwork for: ${track.title}`);
                    }
                  }
                } catch (error) {
        ...
        for (const ev of upcoming.slice(0, 10)) {
          ...
          div.innerHTML = `
            ...
            <div class="event-details">
              ...
              <div class="event-sub">${[ev.artist, ev.album].filter(Boolean).join(' • ') || 'Unknown artist'}</div>
            </div>`;
          sec.appendChild(div);
      ...
      if (live) {
        ...
        card.innerHTML = `
          ...
          <div class="event-details">
            ...
            <div class="event-sub">${[live.artist, live.album].filter(Boolean).join(' • ') || 'Unknown artist'}</div>
          </div>`;
        ...
        if (remain != null) {
          ...
          const t = setInterval(() => {
            remain -= 1000;
            if (remain <= 0) { chip.textContent = 'ending…'; clearInterval(t); return; }
            chip.textContent = `${fmtRemaining(remain)} left`;
          }, 1000);
        } else {
          chip.textContent = '—';
          ...
          if (ev.type === 'dj') {
            div.innerHTML = `
              <div class="event-details" style="width:100%">
                ...
                <div class="event-title">🎙️ ${ev.text || '(DJ line)'}</div>
...
// Update your manual DJ button handler in index.html:
// Debug DJ status button
$('debugDjBtn').onclick = async () => {
  try {
    const result = await j('/api/debug-dj-status');
    console.log('DJ Debug Status:', result);
    
    const summary = `
DJ Status:
- Auto DJ Enabled: ${result.config.auto_dj_enabled}
- Script Exists: ${result.config.script_exists}
- Recent DJ Events: ${result.recent_dj_events.length}
- TTS Files: ${result.tts_files.length}
- Queue Length: ${result.liquidsoap_queue_length}
...
$('genBtn').onclick = async () => {
  try {
    ...
    if (result.skipped) {
      // Show that we skipped because one was recently generated
      $('genBtn').textContent = 'Already Generated!';
      $('genBtn').style.backgroundColor = '#ffb020'; // Warning color
      
      console.log(`Skipped: ${result.message}`);
      console.log(`Recent DJ line: ${result.recent_dj}`);
    } else {
      // Show success for new generation
      $('genBtn').textContent = 'Generated!';
      $('genBtn').style.backgroundColor = '#3ddc97'; // Success color
      
      // Immediately refresh the timeline to show the new DJ line
      loadTimeline();
    }
    ...
    setTimeout(() => {
      $('genBtn').disabled = false;
      $('genBtn').textContent = '✨ Generate DJ Line';
      $('genBtn').style.backgroundColor = '';
    }, 3000);
  }
};
